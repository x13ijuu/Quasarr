...
- **FF** — movie-only companion to the search source. `/external` URLs are resolved manually (`allow_redirects=False`, max 8 hops, loop/404 detection) and resolution returns as soon as a crypter is detected or the redirect leaves the configured FF host, so hoster/crypter pages are not probed. Release-page and movie-token API requests stay plain unless a current response is a Cloudflare block; the optional fallback reuses one lazily created FlareSolverr session for the grab and always destroys it afterward. The requested release title is matched exactly, and the first requested mirror is resolved.
...
- **SF** — two URL modes: `/external` redirect URLs are resolved manually (`allow_redirects=False`, max 8 hops, loop detection, 404 detection) and resolution stops as soon as `detect_crypter_type()` recognizes the URL — the crypter page itself is never loaded. Series-page and season-API requests stay plain unless a current response is a Cloudflare block; the optional fallback reuses one lazily created FlareSolverr session for the grab and always destroys it afterward. Series pages are matched via a strict release-title regex against the season API.
