...
- `DownloadRelease`: `{links: [[url, mirror]] or [[url, mirror, state_url]], title?, password?, imdb_id?, protected?}`. `protected: true` means the returned source-page URL itself requires a manual CAPTCHA and must be parked in the protected queue instead of treated as a direct hoster link. Empty/missing `links` means "no match" and the orchestrator tries the next candidate; return `{"links": []}` on failure rather than raising.
...
- **DD** - searches the cursor-paginated `/api/releases/search` endpoint by `keyword` (the release title) and matches the result whose `releaseName` equals the title; applies its own hostname-based mirror filter; dedupes multiple `.mkv` links per hoster; the API's `fake` release flag triggers session re-creation and abort.
...
- **HE** - links sit behind a content-protector access form that the module extracts (including JS-injected payload fields) and submits to unlock them; the unlocked links are direct, with a FlareSolverr retry loop (up to 3 attempts) when the plain fetch is challenged. A non-empty `captcha_id` is an immediate human-CAPTCHA signal; a completed form submission that still yields no links is the fallback signal after automatic retries. Unparseable page HTML is atomically saved as the latest `HE_unparsed.html` in the configured path, and its full filename is logged. Either returns the source page with `protected: true` for the `/captcha/he` userscript flow; an initial block or missing form still fails normally. Mirrors are filtered by normalized hostname against the configured mirror list only after direct links are unlocked.
